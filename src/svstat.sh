#!/bin/bash
# Do as much as possible of SVSTAT starting after reads are simulated
# external requires:
# -bwa
# -samtools
# -java
...
#      t/_FASTA/derAB_##.*	- bwa, samtools, and fasta files for candidate junctions search
#          t/cj_report.txt	- result!
#
# versions:
...
#find partial alignments sharing start or end coordinates (stacks), filtering same-start+orientation duplicate readIDs, which only happens with flowsim simulated reads
join -j1 -o 2.1,2.2,1.2,2.3,2.4,2.5,2.6,2.7,2.8 <(sort +0 -1 read_len.txt) <(bamtobl.pl 4.bam | sort +0 -1) | awk 'BEGIN{plchldr_cols="1.2 1.4 1.5 1.6 1.7 1.8 1.9"}{ss=$6; se=$7; qs=$4; qe=$5; qlen=$3; ori="+"; if ($8!=1) {ori="-";tmp=qs; qs=qlen-qe+1; qe=qlen-tmp+1;} if (qs!=1) print $0,plchldr_cols,"start"; if (qe!=qlen) print $0,plchldr_cols,"end";}' | tee onehit_stackedreads.txt | sort -k1,1 -k17 -u | awk '{if ($17=="start") print $2,$6,$17; else print $2,$7,$17;}' | sort | uniq -c | awk '{if ($1>1) print $2,$3,$4;}' | tee >(grep end$ > fstack_coord.txt) | grep start$ > rstack_coord.txt;
...
for file in */stackedreads.txt; do cat $file | cut -d" " -f1 | sort | uniq | cdbyank pass_qc.fastq.cidx > ${file}.fastq; done;
