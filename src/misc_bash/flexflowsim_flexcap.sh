...
##4 ""
clonesim -c $lognorm_numreads $f --lengths="LogNormal 3.9 0.2" | FastaToTbl | capture -p <(FastaToTbl ~/goldenpath/hg18/leukseqcap/sim/test/capture_prob/frac_ALLXMATCH.txt.cov.fasta) | TblToFasta | kitsim | mutator | flowsim --generation=Titanium -o output.sff
...
##4 ""
clonesim -c $uniform_numreads $f --lengths="Uniform 65 400" | FastaToTbl | capture -p <(FastaToTbl ~/goldenpath/hg18/leukseqcap/sim/test/capture_prob/frac_ALLXMATCH.txt.cov.fasta) | TblToFasta | kitsim | mutator | flowsim --generation=Titanium -o output.sff
flower output.sff --fasta=output.fasta --fastq=output.fastq
grep ">" output.fasta | sed 's/>[^\.]*\.\.\(.*\)/\1/' > readlengths.txt
...
##4 ""
clonesim -c $gauss_numreads $f --lengths="Normal 390 55" | FastaToTbl | capture -p <(FastaToTbl ~/goldenpath/hg18/leukseqcap/sim/test/capture_prob/frac_ALLXMATCH.txt.cov.fasta) | TblToFasta | kitsim | mutator | flowsim --generation=Titanium -o output.sff
flower output.sff --fasta=output.fasta --fastq=output.fastq
grep ">" output.fasta | sed 's/>[^\.]*\.\.\(.*\)/\1/' > readlengths.txt
...
##3 clonesim -c $gauss_numreads ~/goldenpath/hg18/leukseqcap/sample4_capture_target.fasta --lengths="Normal 390 55" | kitsim | flowsim --generation=Titanium -o output.sff
##4 ""
clonesim -c $uniform_mid_numreads $f --lengths="LogNormal 5.7 0.3" | FastaToTbl | capture -p <(FastaToTbl ~/goldenpath/hg18/leukseqcap/sim/test/capture_prob/frac_ALLXMATCH.txt.cov.fasta) | TblToFasta | kitsim | mutator | flowsim --generation=Titanium -o output.sff
flower output.sff --fasta=output.fasta --fastq=output.fastq
grep ">" output.fasta | sed 's/>[^\.]*\.\.\(.*\)/\1/' > readlengths.txt
