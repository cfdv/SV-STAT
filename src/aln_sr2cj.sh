...
$BWA_BIN_DIR/bwa index -a bwtsw derAB.fasta || $BWA_BIN_DIR/bwa index -a is derAB.fasta;
...
$BEDTOOLS_BIN_DIR/bamToBed -i 1.bam | tee -i 1.bed | awk '{cjID=$1; start=$2; end=$3; mq=$5; lb=501-start; rb=end-500; if (lb<rb) minb=lb; else minb=rb; if (minb < 0) minb = 0; print $1,$2,$3,$4,$5, minb, mq, minb*mq;}' | tee -i raw_cj_supp.txt | awk '{cjID=$1; b=$6; q=$7; bq=$8; bs[cjID]+=b; qs[cjID]+=q; bqs[cjID]+=bq;}END{for (cjID in bs){if (bqs[cjID]!=0) print cjID, bs[cjID], qs[cjID], bqs[cjID], log(bqs[cjID])/log(10);};}' | sort +3 -4 -rn > cj_report.txt;
